...
```{r}
Canada_China_gdp <- gapminder %>%
  filter(country == c("Canada","China"))
ggplot(Canada_gdp, aes(x= year, y= gdpPercap)) + geom_point()
```
...
Canada_gdp <- gapminder %>%
  filter(country == c("Canada","China"))
ggplot(Canada_gdp, aes(x= year, y= gdpPercap, fill=country, color = country)) + geom_point() + ggtitle("GDP per Capitia Growth of China and Canada") + geom_line()
```
